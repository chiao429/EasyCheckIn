  ...
  const getSquadLabel = (attendee: Attendee | null) => {
    ...
    return (
      <p className="text-sm text-slate-800 mt-2 font-semibold">
        {first.標題 || '所屬小隊'}：
        <span className="ml-1 text-slate-900 text-base font-bold">
          {first.值 || '未填寫'}
        </span>
      </p>
    );
          ...
          <CardContent className="space-y-4">
            ...
            {!staffName.trim() && (
              <p className="text-xs text-amber-700 bg-amber-50 border border-amber-200 rounded-md px-3 py-2">
                請先輸入工作人員名稱，之後即可使用下方的搜尋與代為簽到功能。名稱會保存在這台裝置的瀏覽器中，下次開啟會自動帶入。
              </p>
            )}
